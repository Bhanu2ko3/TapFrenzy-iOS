...
class StatsVM: ObservableObject {
    ...
    @Published var gameHistory: [HubGameSession] = []
    
    func refreshStatistics() {
        let rawLedger = UserDefaults.standard.string(forKey: "hub_ledger") ?? "[]"
        let loadedHistory = [HubGameSession].deserialize(from: rawLedger)
        gameHistory = loadedHistory.sorted(by: { $0.playedAt > $1.playedAt })
        
        totalGamesPlayed = loadedHistory.count
        totalAccumulatedPoints = loadedHistory.reduce(0) { $0 + $1.finalScore }
        
        tapFrenzyBest = loadedHistory.filter { $0.mode == .frenzySpeed }.map { $0.finalScore }.max() ?? 0
        lightItUpBest = loadedHistory.filter { $0.mode == .gridMatch }.map { $0.finalScore }.max() ?? 0
        quizRushBest = loadedHistory.filter { $0.mode == .triviaQuiz }.map { $0.finalScore }.max() ?? 0
    }
